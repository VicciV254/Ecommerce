...
const IMG: Record<string, number[]> = {
  "mens-fashion": [3214788, 3209639, 20143795, 19915581, 4452379, 19568392, 19915603, 3649765, 9731828, 20570113, 14564841, 27897903],
  "womens-fashion": [8675179, 36990983, 3209624, 13055401, 8281773, 20453359, 6071513, 18533667, 9322333, 20164322, 9475959, 8675179],
  "childrens-wear": [5693891, 15304383, 9322333, 37305530, 21967197, 13526559, 5693889, 8435759, 9521668, 37242420],
  "baby-nursery": [20387764, 32299511, 15427723, 34618519, 8125846, 21642995, 20387931, 38008350, 35926637, 9534281],
  "home-kitchen": [5556176, 35828604, 6246099, 16927367, 31557569, 16927363, 13969207, 14207018, 13969211, 27569266],
  "home-decor": [20557234, 5793641, 32631105, 18375891, 33316045, 11112732, 16805301, 27556611, 37936133, 23116038],
  electronics: [7989741, 3394650, 4765366, 4526407, 11120516, 8346916, 18311092, 17984647, 36680544, 20321375],
  "school-office": [8230968, 29765813, 13583359, 37793421, 29765799, 14759603, 29765807, 20140155, 29765795, 29765797],
  footwear: [10259873, 27113470, 27988923, 27256470, 4161710, 27875070, 27988921, 8313383, 27100548, 37305530, 27988922, 10259873],
  "outdoor-leisure": [20728294, 20728218, 4061694, 9558021, 32100113, 17098879, 16778082, 9078472, 28810901, 20811927],
};

/* ------------------------------------------------------------------
   NAME-BASED IMAGE OVERRIDES (hosted studio product photos).
   Any product whose exact name matches a key here uses these images
   for the card, hero, and all gallery angles.
   ------------------------------------------------------------------ */
const NAME_IMAGES: Record<string, string> = {
  ...
  "White Blouse - Lace Detail": "https://my-image-host.victor-f6f.workers.dev/api/img/1782738944063-pasted-1782738939063.png",
  ...
  "Blazer - Women's": "https://my-image-host.victor-f6f.workers.dev/api/img/1782743934713-pasted-1782743927276.png",
  "Jumpsuit - Casual": "https://my-image-host.victor-f6f.workers.dev/api/img/1782744140929-pasted-1782744136483.png",
  "Scarf - Silk Blend": "https://my-image-host.victor-f6f.workers.dev/api/img/1782744203501-pasted-1782744199007.png",
  // Children's Wear
  ...
  "Kids' School Shoes": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756836516-pasted-1782756587693.png",
  "Men's Sport Sneakers": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756837542-pasted-1782756627517.png",
  "Women's Flats - Ballet": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756838772-pasted-1782756657520.png",
  "Kids' Sandals": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756840711-pasted-1782756691846.png",
  "Men's Boots - Casual": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756842540-pasted-1782756728131.png",
  "Women's Wedge Sandals": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756843661-pasted-1782756779634.png",
  "Kids' Sneakers - Light-up": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756845474-pasted-1782756823764.png",
  "Men's Formal Oxford": "https://my-image-host.victor-f6f.workers.dev/api/img/1782756881408-pasted-1782756877267.png",
  // Outdoor & Leisure
...
function build(catName: string, catSlug: string, items: Raw[]): Product[] {
  const pool = IMG[catSlug] ?? IMG["mens-fashion"];
  return items.map((it, i) => {
    const compare = Math.round((it.price * (1.15 + (i % 3) * 0.1)) / 100) * 100;
    const rating = 3.5 + ((i * 7) % 15) / 10;
    const img = NAME_IMAGES[it.name] ?? PX(pool[i % pool.length]);
    return {
      id: `${catSlug}-${i + 1}`,
      name: it.name,
      price: it.price,
